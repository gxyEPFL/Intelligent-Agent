...
public class C {
    public static final double P = 0.4; //  A value of p from 0.3 to 0.5 would be a good choice.
    public static ArrayList<Vehicle> agentVehicles; //global
    public static ArrayList<Task> worldTasks;  //global
    ...
    public static Topology.City getCityForAction (int index) {
    ...
    public static Action makeLogistAction (int index) {
    ...
    public static void setupOnce (List<Vehicle> v, TaskSet t) {
        agentVehicles = new ArrayList<Vehicle> (v);
        worldTasks = new ArrayList<>(t);
        worldTasks.sort(taskComparatorId); //sort so we can iterate over tasks in increasing order of ID
    }

    public static C selectInitialSolution() {
        C a = new C();
        a.reset();
    ...
    public static C localChoice(C oldSolution, ArrayList<C> SolutionSet) {
        ...
        SolutionSet.removeIf(s -> s.constraints() == false);
        ...
        SolutionSet.removeIf(s -> s.getCost() > minCost);  //only leaves those with minimum cost
        ...
        if (willChange < P) { //Then with probability p it returns A, with probability 1 − p it returns the current assignment Aold
    ...
    public double getCost() {
        double cost = 0;
        int length=agentVehicles.size();
        for (int i=0;i<length;i++) {
            Topology.City current =agentVehicles.get(i).getCurrentCity();
            int taskAction=firstAction[i];
            if (taskAction==C.NULL) {
                continue;
            }
            while (taskAction != NULL) {
                cost += current.distanceTo(getCityForAction(taskAction));
